...
- **Exercise media** — on first start a one-shot `media` container downloads roughly 140 MB of exercise images and animations from [hasaneyldrm/exercises-dataset](https://github.com/hasaneyldrm/exercises-dataset) into `${APP_DATA_DIR}/data/media`. The app only becomes reachable once that finishes, so the very first start takes a few minutes. Later starts skip the download.
